%
% @author Michal Koutný <michal@fykos.cz>
%
% @description Manipulation with signrature "database".
%
\RequirePackage{etoolbox}

\newcommand\newsignature[5]{%
  \expandafter\def\csname sign@me#1\endcsname{%
    #2%
  }%
  \expandafter\def\csname sigm@il#1\endcsname{%
    #3%
  }%
  \expandafter\def\csname sigs@m#1\endcsname{%
    #4%
  }%
  \expandafter\def\csname sig@nick#1\endcsname{%
    #4%
  }%
}

\newcommand\signame[1]{%
  \expandafter\ifx\csname sign@me#1\endcsname\relax
  \PackageWarning{fkssignature}{Undefined name #1}
  \else
  \expandafter\csname sign@me#1\endcsname\fi
}
\newcommand\sigmail[1]{%
  \expandafter\csname sigm@il#1\endcsname
}
\newcommand\sigsem[1]{%
  \expandafter\csname sigs@m#1\endcsname
}
\newcommand\signick[1]{%
  \expandafter\csname signick#1\endcsname
}

\newcommand\sign@tureraw[1]{%
\signame{#1} <\sigmail{#1}>, %
}

%\DeclareOption{fykos}{%  
	\newcommand*{\sign@ture}[1]{%
	\hspace{10pt}
	\begin{tabular}{c}
	\sigstyle{\signame{#1}}\\
	\ifthenelse{\equal{\sigsem{#1}}{vyfuk}}{\mail{\sigmail{#1}}}{\mail{\sigmail{#1}}}
	\end{tabular}}

\newcommand*{\signature}[1]{%
  \let\do\sign@ture
  \hfill\expandafter\docsvlist\expandafter{#1}
}

\newcommand*{\signatureraw}[1]{%
  \let\do\sign@tureraw
  {\expandafter\docsvlist\expandafter{#1}}
}

% signature style
\newcommand\sigstyle[1]{{\bfseries\itshape#1}}



\input{fksemails}
